...
bool llama_kv_cache::can_use_sparse_attn_plan(const llama_ubatch & ubatch, const slot_info & sinfo) const {
    const char * env = std::getenv("LLAMA_KVFLASH_SPARSE_ATTN");
    if (!env || env[0] == '0' || env[0] == '\0') {
        return false;
    }
    if (other || v_trans || sinfo.n_stream() != 1 || ubatch.n_tokens == 0 || ubatch.n_seq_id[0] != 1 || ubatch.n_seqs_unq != 1) {
        LLAMA_LOG_DEBUG("%s: KVFlash sparse attention plan rejected: other=%d v_trans=%d n_stream=%u sinfo_stream=%zu n_tokens=%u n_seq_id0=%d n_seqs_unq=%u\n",
                __func__, other != nullptr, v_trans, n_stream, sinfo.n_stream(), ubatch.n_tokens, ubatch.n_tokens ? ubatch.n_seq_id[0] : -1, ubatch.n_seqs_unq);
        return false;
    }
    const llama_seq_id seq_id = ubatch.seq_id[0][0];
    for (uint32_t i = 0; i < ubatch.n_tokens; ++i) {
        if (ubatch.n_seq_id[i] != 1 || ubatch.seq_id[i][0] != seq_id) {
            LLAMA_LOG_DEBUG("%s: KVFlash sparse attention plan rejected: mixed seq ids at token %u\n", __func__, i);
            return false;
        }
    }
    const char * env_mrope = std::getenv("LLAMA_KVFLASH_SPARSE_ATTN_MROPE");
    const bool allow_mrope = env_mrope && env_mrope[0] != '0' && env_mrope[0] != '\0';
    if (hparams.use_alibi || (hparams.n_pos_per_embd() != 1 && !allow_mrope)) {
        LLAMA_LOG_DEBUG("%s: KVFlash sparse attention plan rejected: n_swa=%u alibi=%d n_pos_per_embd=%u allow_mrope=%d\n",
                __func__, n_swa, hparams.use_alibi, hparams.n_pos_per_embd(), allow_mrope);
        return false;
    }
    return true;
}

std::vector<uint32_t> llama_kv_cache::build_sparse_attn_plan(const llama_ubatch & ubatch, const slot_info & sinfo) const {
    std::vector<uint32_t> idxs;
    if (!can_use_sparse_attn_plan(ubatch, sinfo)) {
        return idxs;
    }

    const llama_seq_id seq_id = ubatch.seq_id[0][0];
    const auto & cells = v_cells[sinfo.strm[0]];
    idxs.reserve(cells.get_used());
    for (uint32_t i = 0; i < cells.size(); ++i) {
...
ggml_tensor * llama_kv_cache::get_k_sparse(ggml_context * ctx, int32_t il, const slot_info & sinfo, ggml_tensor * idxs) const {
    GGML_UNUSED(sinfo);
    const int32_t ikv = map_layer_ids.at(il);
    auto * k = layers[ikv].k_stream[sinfo.s0];
    const int64_t n_sparse = idxs->ne[0];
    const int64_t n_embd_k_gqa = k->ne[0];
    const int64_t kv_size = get_size();
    GGML_ASSERT(n_embd_k_gqa == hparams.n_embd_k_gqa(il));
    ggml_tensor * rows = ggml_get_rows(ctx, ggml_view_2d(ctx, k, n_embd_k_gqa, kv_size, k->nb[1], 0), idxs);
    return ggml_reshape_4d(ctx, rows, hparams.n_embd_head_k(il), hparams.n_head_kv(il), n_sparse, 1);
}

ggml_tensor * llama_kv_cache::get_v_sparse(ggml_context * ctx, int32_t il, const slot_info & sinfo, ggml_tensor * idxs) const {
    GGML_UNUSED(sinfo);
    const int32_t ikv = map_layer_ids.at(il);
    auto * v = layers[ikv].v_stream[sinfo.s0];
    const int64_t n_sparse = idxs->ne[0];
    const int64_t n_embd_v_gqa = v->ne[0];
    const int64_t kv_size = get_size();
    GGML_ASSERT(!v_trans);
    GGML_ASSERT(n_embd_v_gqa >= hparams.n_embd_v_gqa(il));
    ggml_tensor * rows = ggml_get_rows(ctx, ggml_view_2d(ctx, v, n_embd_v_gqa, kv_size, v->nb[1], 0), idxs);
    return ggml_reshape_4d(ctx, rows, hparams.n_embd_head_v(il), hparams.n_head_kv(il), n_sparse, 1);
}
...
ggml_tensor * llama_kv_cache::build_input_sparse_attn_idxs(ggml_context * ctx, uint32_t n_kv) const {
    ggml_tensor * idxs = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, n_kv);
    ggml_set_input(idxs);
    ggml_set_name(idxs, "attn_inp_sparse_idxs");
    return idxs;
}
...
static bool llama_kv_cache_sparse_mask_keep(
    ...
    if (n_swa != 0 && llama_hparams::is_masked_swa(n_swa, swa_type, p0, p1)) {
        return false;
    }
    return true;
}

void llama_kv_cache::set_input_kq_mask_sparse(ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn, const std::vector<uint32_t> & idxs, uint32_t n_used) const {
    const uint32_t n_tokens = ubatch->n_tokens;
    GGML_ASSERT(ggml_backend_buffer_is_host(dst->buffer));
    GGML_ASSERT(dst->ne[0] == (int64_t) idxs.size());
    GGML_ASSERT(dst->ne[1] == n_tokens);
    GGML_ASSERT(dst->ne[3] == 1);
    GGML_ASSERT(causal_attn);

    const bool is_2d = ubatch->is_pos_2d();
    const llama_seq_id seq_id = ubatch->seq_id[0][0];
    const auto & cells = v_cells[seq_to_stream[seq_id]];
    const char * env_verify = std::getenv("LLAMA_KVFLASH_SPARSE_ATTN_VERIFY_MASK");
    const bool verify_mask = env_verify && env_verify[0] != '0' && env_verify[0] != '\0';
    uint64_t verify_checked = 0;
    uint64_t verify_mismatches = 0;

    for (uint32_t i = 0; i < n_tokens; ++i) {
        const llama_pos p1 = ubatch->pos[i];
        const llama_pos p1_x = is_2d ? ubatch->pos[i + n_tokens*2] : 0;
        const llama_pos p1_y = is_2d ? ubatch->pos[i + n_tokens]   : 0;
        for (uint32_t jj = 0; jj < idxs.size(); ++jj) {
            const uint32_t j = idxs[jj];
            const bool keep_ref = jj < n_used && llama_kv_cache_sparse_mask_keep(cells, seq_id, j, p1, p1_x, p1_y, is_2d, n_swa, swa_type);
            const bool keep = keep_ref;
            if (verify_mask && jj < n_used) {
                const bool keep_dense_equiv = llama_kv_cache_sparse_mask_keep(cells, seq_id, j, p1, p1_x, p1_y, is_2d, n_swa, swa_type);
                verify_checked++;
                verify_mismatches += keep_dense_equiv != keep_ref;
            }
            const float value = keep ? 0.0f : -INFINITY;
            if (dst->type == GGML_TYPE_F16) {
                ((ggml_fp16_t *) dst->data)[i*idxs.size() + jj] = ggml_fp32_to_fp16(value);
            } else {
                ((float *) dst->data)[i*idxs.size() + jj] = value;
            }
        }
    }

    if (verify_mask) {
        LLAMA_LOG_WARN("%s: KVFlash sparse mask verification checked=%llu mismatches=%llu is_2d=%d n_used=%u n_sparse=%zu\n",
                __func__, (unsigned long long) verify_checked, (unsigned long long) verify_mismatches, is_2d, n_used, idxs.size());
        GGML_ASSERT(verify_mismatches == 0);
    }
}
...
bool llama_kv_cache_context::apply() {
    ...
    sparse_attn_idxs = kv->build_sparse_attn_plan(ubatches[i_cur], sinfos[i_cur]);
    sparse_attn_n_used = sparse_attn_idxs.size();
    if (!sparse_attn_idxs.empty()) {
        const uint32_t n_pad_sparse = 64;
        sparse_attn_idxs.resize(GGML_PAD((uint32_t) sparse_attn_idxs.size(), n_pad_sparse), sparse_attn_idxs.back());
    }
    n_kv = sparse_attn_idxs.empty() ? kv->get_n_kv(sinfos[i_cur]) : (int32_t) sparse_attn_idxs.size();
    if (!sparse_attn_idxs.empty()) {
        LLAMA_LOG_WARN("%s: KVFlash sparse attention plan active: gathered_cells=%zu\n", __func__, sparse_attn_idxs.size());
    }
...
bool llama_kv_cache_context::has_sparse_attn_plan() const {
    return !sparse_attn_idxs.empty();
}

ggml_tensor * llama_kv_cache_context::build_input_sparse_attn_idxs(ggml_context * ctx) const {
    return kv->build_input_sparse_attn_idxs(ctx, n_kv);
}

ggml_tensor * llama_kv_cache_context::get_k_sparse(ggml_context * ctx, int32_t il, ggml_tensor * idxs) const {
    return kv->get_k_sparse(ctx, il, sinfos[i_cur], idxs);
}

ggml_tensor * llama_kv_cache_context::get_v_sparse(ggml_context * ctx, int32_t il, ggml_tensor * idxs) const {
    return kv->get_v_sparse(ctx, il, sinfos[i_cur], idxs);
}
...
void llama_kv_cache_context::set_input_sparse_attn_idxs(ggml_tensor * dst) const {
    GGML_ASSERT(ggml_backend_buffer_is_host(dst->buffer));
    GGML_ASSERT(dst->ne[0] == (int64_t) sparse_attn_idxs.size());
    int32_t * data = (int32_t *) dst->data;
    for (size_t i = 0; i < sparse_attn_idxs.size(); ++i) {
        data[i] = (int32_t) sparse_attn_idxs[i];
    }
}

void llama_kv_cache_context::set_input_kq_mask(ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn) const {
    if (!sparse_attn_idxs.empty()) {
        kv->set_input_kq_mask_sparse(dst, ubatch, causal_attn, sparse_attn_idxs, sparse_attn_n_used);
    } else {
