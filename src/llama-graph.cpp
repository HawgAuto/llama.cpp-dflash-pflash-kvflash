...
void llm_graph_input_attn_kv::set_input(const llama_ubatch * ubatch) {
    ...
    if (self_sparse_idxs && self_sparse_idxs->buffer) {
        mctx->set_input_sparse_attn_idxs(self_sparse_idxs);
    }
...
bool llm_graph_input_attn_kv::can_reuse(const llm_graph_params & params) {
  ...
  //res &= self_v_idxs->ne[0] == params.ubatch.n_tokens; // TODO: need to move this to the unified cache and check there
    ...
    res &= (self_sparse_idxs != nullptr) == mctx->has_sparse_attn_plan();
    if (self_sparse_idxs) {
        res &= self_sparse_idxs->ne[0] == mctx->get_n_kv();
    }
...
void llm_graph_input_mem_hybrid::set_input(const llama_ubatch * ubatch) {
    ...
    if (inp_attn->self_sparse_idxs && inp_attn->self_sparse_idxs->buffer) {
        mctx->get_attn()->set_input_sparse_attn_idxs(inp_attn->self_sparse_idxs);
    }
...
bool llm_graph_input_mem_hybrid::can_reuse(const llm_graph_params & params) {
  ...
  //res &= inp_attn->self_v_idxs->ne[0] == params.ubatch.n_tokens; // TODO: need to move this to the unified cache and check there
    ...
    res &= (inp_attn->self_sparse_idxs != nullptr) == mctx->get_attn()->has_sparse_attn_plan();
    if (inp_attn->self_sparse_idxs) {
        res &= inp_attn->self_sparse_idxs->ne[0] == mctx->get_attn()->get_n_kv();
    }
...
static std::unique_ptr<llm_graph_input_attn_kv> build_attn_inp_kv_impl(
...

    {
        ...
        if (mctx_cur->has_sparse_attn_plan()) {
            inp->self_sparse_idxs = mctx_cur->build_input_sparse_attn_idxs(ctx0);
        }
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (inp->get_sparse_idxs()) {
        k = mctx_cur->get_k_sparse(ctx0, il, inp->get_sparse_idxs());
        v = mctx_cur->get_v_sparse(ctx0, il, inp->get_sparse_idxs());
        cb(k, "kvflash_sparse_k", il);
        cb(v, "kvflash_sparse_v", il);
    } else {
