...
std::vector<std::unique_ptr<field>> make_llama_cmpl_schema(const common_params & params_base, task_params & params) {
    ...
    add((new field_num("speculative.n_max", params.speculative.draft.n_max))
        ->set_hard_limits(0, INT32_MAX)
        ->add_alias("dflash.n_max")
        ->set_desc("Maximum number of draft tokens to request for this completion. This can lower the server default per request, including draft-dflash decode gears."));

    add((new field_nested("speculative"))
        ->add_subfield((new field_num("n_max", params.speculative.draft.n_max))
            ->set_hard_limits(0, INT32_MAX)
            ->set_desc("Maximum number of draft tokens to request for this completion."))
        ->set_desc("Speculative decoding options for this request"));

    add((new field_nested("dflash"))
        ->add_subfield((new field_num("n_max", params.speculative.draft.n_max))
            ->set_hard_limits(0, INT32_MAX)
            ->set_desc("Maximum number of draft tokens to request for draft-dflash."))
        ->set_desc("DFlash options for this request"));

    add((new field_num("pflash.keep_ratio", params.pflash_keep_ratio))
        ->set_hard_limits(0.05, 1.0)
        ->add_alias("pflash_keep_ratio")
        ->set_desc("PFlash prompt-compression keep ratio for this request. Values below 1.0 can override the startup default without restarting the server."));

    add((new field_num("pflash.min_keep_tokens", params.pflash_min_keep_tokens))
        ->set_hard_limits(2, INT32_MAX)
        ->add_alias("pflash_min_keep_tokens")
        ->set_desc("Minimum number of prompt tokens PFlash must preserve for this request."));

    add((new field_num("pflash.chat_min_tokens", params.pflash_chat_min_tokens))
        ->set_hard_limits(0, INT32_MAX)
        ->add_alias("pflash_chat_min_tokens")
        ->set_desc("Minimum chat-formatted prompt length before PFlash compression is considered for this request."));

    add((new field_nested("pflash"))
        ->add_subfield((new field_num("keep_ratio", params.pflash_keep_ratio))
            ->set_hard_limits(0.05, 1.0)
            ->set_desc("PFlash prompt-compression keep ratio for this request."))
        ->add_subfield((new field_num("min_keep_tokens", params.pflash_min_keep_tokens))
            ->set_hard_limits(2, INT32_MAX)
            ->set_desc("Minimum number of prompt tokens PFlash must preserve for this request."))
        ->add_subfield((new field_num("chat_min_tokens", params.pflash_chat_min_tokens))
            ->set_hard_limits(0, INT32_MAX)
            ->set_desc("Minimum chat-formatted prompt length before PFlash compression is considered for this request."))
        ->set_desc("PFlash prompt-compression options for this request"));
...
task_params eval_llama_cmpl_schema(
    ...
    params.pflash_keep_ratio      = params_base.pflash_keep_ratio;
    params.pflash_chat_min_tokens = params_base.pflash_chat_min_tokens;
    params.pflash_min_keep_tokens = params_base.pflash_min_keep_tokens;
