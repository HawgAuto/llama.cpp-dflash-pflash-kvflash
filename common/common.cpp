...
void common_params_print_info(const common_params & params, bool print_devices) {
    ...
    if (params.kvflash_tokens != 0 || params.pflash_mode != "off" || !params.pflash_drafter.empty() || !params.pflash_model.empty()) {
        COM_INF("%s: experimental Lucebox ports: kvflash=%d policy=%s tau=%d pflash_mode=%s pflash_keep_ratio=%.3f pflash_score=%s pflash_chat_min=%d pflash_min_keep=%d pflash_edges=%d/%d pflash_latest_user=%s:%d pflash_middle_only=%s\n",
                __func__, params.kvflash_tokens, params.kvflash_policy.c_str(), params.kvflash_tau,
                params.pflash_mode.c_str(), params.pflash_keep_ratio, params.pflash_score.c_str(),
                params.pflash_chat_min_tokens, params.pflash_min_keep_tokens,
                params.pflash_protected_prefix, params.pflash_protected_suffix,
                params.pflash_latest_user_mode.c_str(), params.pflash_latest_user_tail_tokens,
                params.pflash_compress_middle_only ? "true" : "false");
        if (!params.kvflash_drafter.empty()) {
            COM_INF("%s: experimental kvflash_drafter = %s\n", __func__, params.kvflash_drafter.c_str());
        }
        if (!params.pflash_drafter.empty()) {
            COM_INF("%s: experimental pflash_drafter = %s gpu=%d\n", __func__, params.pflash_drafter.c_str(), params.pflash_drafter_gpu);
        }
