...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--pflash-chat-min-tokens"}, "N",
        string_format("experimental: minimum chat-formatted prompt tokens before PFlash may compress (default: %d)", params.pflash_chat_min_tokens),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("pflash-chat-min-tokens must be non-negative");
            }
            params.pflash_chat_min_tokens = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_CHAT_MIN_TOKENS").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-min-keep-tokens"}, "N",
        string_format("experimental: minimum absolute token budget PFlash must keep (default: %d)", params.pflash_min_keep_tokens),
        [](common_params & params, int value) {
            if (value < 2) {
                throw std::invalid_argument("pflash-min-keep-tokens must be at least 2");
            }
            params.pflash_min_keep_tokens = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_MIN_KEEP_TOKENS").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-protected-prefix"}, "N",
        string_format("experimental: PFlash always-kept prefix tokens (default: %d)", params.pflash_protected_prefix),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("pflash-protected-prefix must be non-negative");
            }
            params.pflash_protected_prefix = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PROTECTED_PREFIX").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-protected-suffix"}, "N",
        string_format("experimental: PFlash always-kept suffix tokens (default: %d)", params.pflash_protected_suffix),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("pflash-protected-suffix must be non-negative");
            }
            params.pflash_protected_suffix = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PROTECTED_SUFFIX").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-latest-user-mode"}, "MODE",
        string_format("experimental: latest user-message preservation mode for PFlash: full or tail (default: %s)", params.pflash_latest_user_mode.c_str()),
        [](common_params & params, const std::string & value) {
            if (value != "full" && value != "tail") {
                throw std::invalid_argument("pflash-latest-user-mode must be one of: full, tail");
            }
            params.pflash_latest_user_mode = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_LATEST_USER_MODE").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-latest-user-tail-tokens"}, "N",
        string_format("experimental: latest-user tail tokens preserved when --pflash-latest-user-mode tail is active (default: %d)", params.pflash_latest_user_tail_tokens),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("pflash-latest-user-tail-tokens must be non-negative");
            }
            params.pflash_latest_user_tail_tokens = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_LATEST_USER_TAIL_TOKENS").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-preserve-system"},
        {"--no-pflash-preserve-system"},
        string_format("experimental: preserve detected system-message spans during PFlash (default: %s)", params.pflash_preserve_system ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.pflash_preserve_system = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PRESERVE_SYSTEM").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-preserve-latest-user"},
        {"--no-pflash-preserve-latest-user"},
        string_format("experimental: preserve detected latest user-message span during PFlash (default: %s)", params.pflash_preserve_latest_user ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.pflash_preserve_latest_user = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PRESERVE_LATEST_USER").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-preserve-role-boundaries"},
        {"--no-pflash-preserve-role-boundaries"},
        string_format("experimental: preserve detected chat role/control boundary spans during PFlash (default: %s)", params.pflash_preserve_role_boundaries ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.pflash_preserve_role_boundaries = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PRESERVE_ROLE_BOUNDARIES").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-preserve-control-tokens"},
        {"--no-pflash-preserve-control-tokens"},
        string_format("experimental: preserve tokenizer control tokens during PFlash (default: %s)", params.pflash_preserve_control_tokens ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.pflash_preserve_control_tokens = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PRESERVE_CONTROL_TOKENS").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-preserve-tool-json"},
        {"--no-pflash-preserve-tool-json"},
        string_format("experimental: preserve detected tool/JSON spans during PFlash (default: %s)", params.pflash_preserve_tool_json ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.pflash_preserve_tool_json = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_PRESERVE_TOOL_JSON").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-compress-middle-only"},
        {"--no-pflash-compress-middle-only"},
        string_format("experimental: restrict PFlash lossy selection to the middle between protected edges (default: %s)", params.pflash_compress_middle_only ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.pflash_compress_middle_only = value;
        }
    ).set_env("LLAMA_ARG_PFLASH_COMPRESS_MIDDLE_ONLY").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--pflash-mode"}, "MODE",
